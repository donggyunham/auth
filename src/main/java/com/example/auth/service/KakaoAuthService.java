...
public class KakaoAuthService {
    ...
    // 대문자는 상수로 인식해서 변경x
    @Value("${oauth.kakao.provider_name:KAKAO}")
    private String PROVIDER_NAME;

    /**
     * 카카오 인가 코드 요청을 보낼 URL 생성
     * 사용자를 카카오 로그인 페이지로 리다이렉션 하기 위한 URL
     *
    ...
    public LoginResponse loginByKakao(KakaoUserResponse kakaoUserResponse) {
        ...
        // 이메일은 필수. email == "" -> isEmpty email == " " -> isBlank
        if (email == null || email.isBlank()) {
            // 이메일을 필수로 처리하지 않는다면 provider와 provider_id를 조합하여 가짜 이메일을 생성한다.
            // 예를들어 KAKAO_xxxxxxx@example.com 을 만들어 email로 활용.
