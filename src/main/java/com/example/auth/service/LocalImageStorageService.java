...
@Slf4j
@Service
@Primary        // (인터페이스)구현체가 여러개 있을 경우 이 인스턴스를를 우선적으로 스프링 프레임워크가 주입시켜준다.
public class LocalImageStorageService implements ImageStorageService{
    ...
    // 최대 파일 크기 설정
    private static final  long MAX_FILE_SIZE = 1024 * 1024 * 70;     // 70MB
    ...
    public ImageUploadResponse store(MultipartFile file) {
        log.info("이미지 업로드 store() - 파일명: {}, 크기: {} bytes", file.getOriginalFilename(), file.getSize());
    ...
    public void delete(String fileName) {
        try {
            ...
            throw new FileStorageException("이미지 삭제중 오류가 발생했습니다.", e);
        }
    }
    ...
    private void validateFile(MultipartFile file) throws InvalidFileException{
        ...
        if(file.getSize() > MAX_FILE_SIZE) {
            throw new InvalidFileException("파일 최대 허용 크기는 70mb입니다.");
        }
